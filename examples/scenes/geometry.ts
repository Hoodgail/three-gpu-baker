import type { Triangle, CoreScene } from '../../src/types.js';
import { geometricNormal } from '../../src/core/math.js';
export function quad(p: number[][], material = 0, owner = 'quad') {
  return [
    [0, 1, 2],
    [0, 2, 3],
  ].map((ids) => {
...
export function planeScene(extra: Partial<CoreScene> = {}): CoreScene {
  return {
...
export function box(center: number[], size: number[], angle = 0, material = 0, owner = 'box') {
  const [x, y, z] = center,
    [w, h, d] = size.map((v) => v / 2),
    c = Math.cos(angle),
    s = Math.sin(angle),
    p = (a: number, b: number, e: number) => [x + a * c + e * s, y + b, z - a * s + e * c];
  return [
    [
      [-w, -h, d],
      [w, -h, d],
    ...
    quad(
      f.map((v) => p(v[0], v[1], v[2])),
      material,
      owner + ':' + i,
    ),
  );
}
export function cornellScene(): CoreScene {
  return {
    materials: [
