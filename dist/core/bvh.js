...
export function buildBVH(triangles, { leafSize = 4, bins = 12 } = {}) {
    if (!Number.isInteger(bins) || bins < 2 || bins > 64)
        throw new RangeError('bins must be 2..64.');
    if (!triangles.length)
        throw new Error('Cannot build a BVH without triangles.');
    ...
    const nodes = [], ordered = [], originalIndices = [];
    function recurse(items, depth = 0) {
        const bounds = emptyBounds(), centroids = emptyBounds();
        ...
        if (items.length <= leafSize || depth >= 48) {
            ...
            for (let axis = 0; axis < 3; axis++) {
                ...
                for (const r of items) {
                    const j = Math.min(bins - 1, Math.floor(((r.c[axis] - centroids.min[axis]) / extent) * bins));
                    extend(buckets[j].b, r.b);
                    buckets[j].n++;
                }
                for (let split = 0; split < bins - 1; split++) {
                    const a = emptyBounds(), b = emptyBounds();
                    let an = 0, bn = 0;
            ...
            let left = [], right = [];
            if (bestAxis >= 0) {
                const edge = centroids.min[bestAxis] +
                    ((centroids.max[bestAxis] - centroids.min[bestAxis]) * (bestSplit + 1)) / bins;
                for (const r of items)
                    (r.c[bestAxis] < edge ? left : right).push(r);
            }
...
export function intersectsBounds(origin, direction, node, maxT, minT = 0) {
    let near = minT, far = maxT;
    for (let k = 0; k < 3; k++) {
        if (Math.abs(direction[k]) < 1e-20) {
            if (origin[k] < node.min[k] || origin[k] > node.max[k])
                return false;
        }
        else {
            let a = (node.min[k] - origin[k]) / direction[k], b = (node.max[k] - origin[k]) / direction[k];
            if (a > b)
                [a, b] = [b, a];
...
export function intersectTriangle(origin, direction, triangle, maxT = Infinity, minT = 1e-7) {
    const e1 = sub(triangle.p[1], triangle.p[0]), e2 = sub(triangle.p[2], triangle.p[0]);
    const p = cross(direction, e2), det = dot(e1, p);
    if (Math.abs(det) < 1e-10)
        return null;
    const inv = 1 / det, t = sub(origin, triangle.p[0]), u = dot(t, p) * inv;
    if (u < 0 || u > 1)
        return null;
...
export function trace(bvh, origin, direction, maxT = Infinity, skip = -1, minT = 1e-7, anyHit = false) {
    let closest = null, index = 0;
...
export function surfaceAt(bvh, hit, rayDirection) {
    const t = bvh.triangles[hit.index], w = hit.bary;
