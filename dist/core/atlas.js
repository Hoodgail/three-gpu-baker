...
const key3 = (p) => p.map((v) => Math.round(v * 1e6)).join(',');
const edgeKey = (a, b) => (a < b ? `${a}|${b}` : `${b}|${a}`);
const inside = (a, b) => a.x >= b.x && a.y >= b.y && a.x + a.w <= b.x + b.w && a.y + a.h <= b.y + b.h;
function chartGroups(triangles, existing = false) {
    ...
    triangles.forEach((t, i) => {
        for (let e = 0; e < 3; e++) {
            const a = e, b = (e + 1) % 3;
            const ka = key3(t.p[a]) +
                (existing ? `/${t.lmUV[a].map((v) => Math.round(v * 1e7)).join(',')}` : ''), kb = key3(t.p[b]) +
...
function pack(charts, width, height, padding, density) {
    ...
    rectangles.sort((a, b) => Math.max(b.w, b.h) - Math.max(a.w, a.h) || b.w * b.h - a.w * a.h || a.id - b.id);
    ...
    if (charts.length > 128)
        return packShelves(rectangles, width, height);
    ...
    for (const rect of rectangles) {
        ...
        for (let i = 0; i < free.length; i++)
            for (let rotate = 0; rotate < 2; rotate++) {
                const w = rotate ? rect.h : rect.w, h = rotate ? rect.w : rect.h, f = free[i];
                if (w > f.w || h > f.h)
                    continue;
                const score = Math.min(f.w - w, f.h - h) * 1e6 + Math.max(f.w - w, f.h - h);
                if (!choice || score < choice.score)
                    choice = { i, w, h, score, rotate: !!rotate };
            }
        if (!choice)
            return null;
        const f = free[choice.i], used = { x: f.x, y: f.y, w: choice.w, h: choice.h };
        const next = [];
        ...
        for (const r of free) {
            ...
            if (used.x > r.x)
                next.push({ x: r.x, y: r.y, w: used.x - r.x, h: r.h });
            ...
            if (used.y > r.y)
                next.push({ x: r.x, y: r.y, w: r.w, h: used.y - r.y });
...
function packShelves(rectangles, width, height) {
    const shelves = [], placed = [];
    let bottom = 0;
    for (const rect of rectangles) {
        let best = null;
        for (let rotate = 0; rotate < 2; rotate++) {
            const w = rotate ? rect.h : rect.w, h = rotate ? rect.w : rect.h;
            if (w > width || h > height)
                continue;
            for (const shelf of shelves)
                if (h <= shelf.h && shelf.x + w <= width) {
                    ...
                    if (!best || score < best.score)
                        best = { shelf, w, h, rotate: !!rotate, score };
                }
        }
        if (!best) {
            const variants = [
            ...
            if (!variants.length)
                return null;
            const r = variants[0], shelf = { x: 0, y: bottom, h: r.h };
            bottom += r.h;
            shelves.push(shelf);
            best = { ...r, shelf };
        }
...
export function generateAtlas(triangles, width, height, { padding = 2, uvMode = 'generate', texelDensity, } = {}) {
    if (!triangles.length)
        throw new Error('Atlas generation requires triangles.');
    ...
    const charts = groups.map((indices, id) => {
        const seed = triangles[indices[0]], normal = geometricNormal(seed), u = normalize(sub(seed.p[1], seed.p[0])), v = cross(normal, u);
        let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
        for (const i of indices)
            for (let k = 0; k < 3; k++) {
                const p = triangles[i].p[k];
                const x = repack ? triangles[i].lmUV[k][0] : dot(p, u), y = repack ? triangles[i].lmUV[k][1] : dot(p, v);
        ...
        return {
            ...
            scale: repack
                ? Math.sqrt(indices.reduce((s, i) => s + triangleArea(triangles[i]), 0) /
                    indices.reduce((s, i) => {
    ...
    if (!pack(charts, width, height, padding, 0))
        throw new Error(`${charts.length} UV charts do not fit ${width}×${height} with ${padding}px padding. Increase resolution or provide an xatlas-generated uv1 atlas.`);
    let lo = 0, hi = Math.sqrt((width * height) / charts.reduce((s, c) => s + c.dx * c.dy * c.scale * c.scale, 0)) * 2;
    if (texelDensity !== undefined)
        lo = texelDensity;
    else {
        while (pack(charts, width, height, padding, hi))
            hi *= 2;
        for (let k = 0; k < 25; k++) {
            const mid = (lo + hi) * 0.5;
            if (pack(charts, width, height, padding, mid))
                lo = mid;
            else
                hi = mid;
        }
    ...
    if (!placed)
        throw new Error('Atlas packing failed at requested texelDensity. Increase resolution or lower density/padding.');
    for (const c of charts) {
        const r = placed[c.id], iw = r.w - 2 * padding, ih = r.h - 2 * padding;
        for (const i of c.indices) {
            result[i].chart = c.id;
            result[i].lmUV = triangles[i].p.map((p, k) => {
                const x = ((repack ? triangles[i].lmUV[k][0] : dot(p, c.u)) - c.minX) / c.dx, y = ((repack ? triangles[i].lmUV[k][1] : dot(p, c.v)) - c.minY) / c.dy;
                return [
...
export function rasterizeAtlas(triangles, materials, width, height, { conservative = false, padding = 0, diagnostics = [], } = {}) {
    ...
    const positions = new Float32Array(size * 3), normals = new Float32Array(size * 3), geometricNormals = new Float32Array(size * 3), albedo = new Float32Array(size * 3);
    const coverage = new Uint8Array(size), charts = new Int32Array(size).fill(-1), triangleIds = new Int32Array(size).fill(-1), interior = new Float32Array(size);
    const chartCoverage = new Map();
    let covered = 0;
    triangles.forEach((t, triangle) => {
        const uv = t.lmUV.map((p) => [p[0] * width, p[1] * height]);
        const [a, b, c] = uv, den = (b[1] - c[1]) * (a[0] - c[0]) + (c[0] - b[0]) * (a[1] - c[1]);
        ...
        for (let y = minY; y <= maxY; y++)
            for (let x = minX; x <= maxX; x++) {
                const px = x + 0.5, py = y + 0.5, w0 = ((b[1] - c[1]) * (px - c[0]) + (c[0] - b[0]) * (py - c[1])) / den, w1 = ((c[1] - a[1]) * (px - c[0]) + (a[0] - c[0]) * (py - c[1])) / den, w2 = 1 - w0 - w1, edge = Math.min(w0, w1, w2);
                if (edge < -1e-6)
                    continue;
                const index = y * width + x, w = [w0, w1, w2];
                if (coverage[index]) {
                    if (charts[index] !== t.chart || (edge > 1e-5 && interior[index] > 1e-5)) {
                        const previous = triangleIds[index], key = `${previous}:${triangle}`;
    ...
    if (conservative)
        for (let m = missing.length - 1; m >= 0; m--) {
            ...
            for (let triangle = 0; triangle < triangles.length; triangle++) {
                ...
                if (t.chart !== chart)
                    continue;
                const x = Math.min(width - 1, Math.floor((t.lmUV.reduce((s, v) => s + v[0], 0) * width) / 3)), y = Math.min(height - 1, Math.floor((t.lmUV.reduce((s, v) => s + v[1], 0) * height) / 3)), index = y * width + x;
                if (coverage[index])
                    continue;
                ...
                const w = [1 / 3, 1 / 3, 1 / 3], g = geometricNormal(t);
                let n = t.n ? normalize(mix3(t.n[0], t.n[1], t.n[2], w)) : g;
                if (dot(n, g) < 0)
                    n = n.map((v) => -v);
                coverage[index] = 1;
    ...
    if (errors.length)
        throw new UVValidationError(errors);
    ...
    if (!covered)
        throw new Error('The atlas has no covered texels.');
    return {
        width,
        height,
        positions,
...
export function validateLightmapUVs(triangles, width, height, padding = 2) {
    if (!Number.isSafeInteger(width) ||
        !Number.isSafeInteger(height) ||
        width < 1 ||
        height < 1 ||
    ...
    try {
        ...
        rasterizeAtlas(atlas.triangles.map((t) => ({ ...t, material: 0 })), [{}], width, height, { padding, diagnostics });
    }
    catch (error) {
...
export function dilateChannels(result, iterations = 2) {
    const out = {
        ...result,
    ...
    for (let i = 0; i < owners.length; i++)
        owners[i] = result.coverage[i] ? i : -1;
            ...
            for (let x = 0; x < result.width; x++) {
                ...
                for (const [dx, dy] of [
                    ...
                    if (source < 0)
                        continue;
                    next[i] = source;
                    for (const key of ['direct', 'indirect', 'lightmap'])
                        for (let k = 0; k < 3; k++)
                            out[key][i * 3 + k] = result[key][source * 3 + k];
