import { RNG, sampleSeed, uniformSphere, shBasis, PI, add, normalize, clamp, abortIfNeeded, yieldToHost, triangleArea, } from './math.js';
...
export function createProbeGrid({ min, max, spacing = 1 }) {
    if (!min ||
        !max ||
        min.length !== 3 ||
        max.length !== 3 ||
    ...
    if (count > 100000)
        throw new RangeError('Probe grid exceeds 100,000 probes. Increase spacing.');
    const positions = [];
    for (let z = 0; z < dimensions[2]; z++)
        for (let y = 0; y < dimensions[1]; y++)
            for (let x = 0; x < dimensions[0]; x++)
                positions.push([x, y, z].map((i, k) => dimensions[k] === 1 ? min[k] : min[k] + (i * (max[k] - min[k])) / (dimensions[k] - 1)));
    return { min: [...min], max: [...max], dimensions, positions };
}
export async function generateLightProbes(input, options = {}) {
    const samples = options.samples ?? 512, bounces = options.bounces ?? ('options' in input ? input.options.bounces : undefined) ?? 2, seed = options.seed ?? 1337;
    if (!Number.isInteger(samples) || samples < 1 || samples > 16777216)
        throw new RangeError('Invalid probe samples.');
    if (!Number.isInteger(bounces) ||
        bounces < 0 ||
        bounces > 8 ||
    ...
    if ('bvh' in input)
        ...
        const core = 'isObject3D' in input
    ...
    const grid = options.grid ? createProbeGrid(options.grid) : null, positions = options.positions ?? grid?.positions;
    if (!positions?.length ||
    ...
    for (let index = 0; index < positions.length; index++) {
        const p = positions[index], coefficients = new Float64Array(27), surface = { p, n: null, g: null, triangle: -1 };
        for (let sample = 0; sample < samples; sample++) {
            ...
            const direct = sampleDirect(scene, surface, rng, { spherical: true }), directBasis = shBasis(direct.direction);
            const direction = uniformSphere(rng), incoming = traceIncoming(scene, surface, direction, rng, {
                initialPDF: 1 / (4 * PI),
                bounces,
            });
            const radiance = add(incoming.direct, incoming.indirect), basis = shBasis(direction);
            for (let k = 0; k < 9; k++)
                for (let c = 0; c < 3; c++)
                    coefficients[k * 3 + c] +=
...
export function validateProbes(data) {
    if (data?.schema !== 'three-lightmap-probes' ||
        data.version !== 1 ||
    ...
    if (data.grid) {
        ...
        if (!g.min ||
            ...
            g.dimensions.some((d, k) => !Number.isInteger(d) || d < 1 || g.max[k] < g.min[k] || (d > 1 && g.max[k] === g.min[k])) ||
            g.dimensions.reduce((a, b) => a * b, 1) !== data.probes.length)
            throw new Error('Invalid probe grid.');
    }
    return data;
}
export function exportProbes(data) {
    validateProbes(data);
    return JSON.stringify(data, null, 2);
}
export function importProbes(text) {
...
export function evaluateSHIrradiance(coefficients, normal) {
    if (coefficients.length !== 27)
        throw new Error('SH9 requires 27 RGB values.');
    const basis = shBasis(normalize(normal)), out = [0, 0, 0];
    for (let k = 0; k < 9; k++) {
        const convolution = k === 0 ? PI : k < 4 ? (2 * PI) / 3 : PI / 4;
        for (let c = 0; c < 3; c++)
            out[c] += coefficients[k * 3 + c] * basis[k] * convolution;
    }
    return out;
}
export function interpolateProbeGrid(data, position) {
    validateProbes(data);
    const grid = data.grid;
    if (!grid)
        throw new Error('Trilinear interpolation requires a regular grid.');
    const { min, max, dimensions: d } = grid;
    const coord = position.map((v, k) => d[k] === 1 ? 0 : clamp(((v - min[k]) / (max[k] - min[k])) * (d[k] - 1), 0, d[k] - 1)), base = coord.map(Math.floor), f = coord.map((v, k) => v - base[k]), out = new Float64Array(27);
    for (let z = 0; z < 2; z++)
        for (let y = 0; y < 2; y++)
            for (let x = 0; x < 2; x++) {
                const index = [x, y, z].map((v, k) => Math.min(d[k] - 1, base[k] + v)), weight = (x ? f[0] : 1 - f[0]) * (y ? f[1] : 1 - f[1]) * (z ? f[2] : 1 - f[2]);
                const p = data.probes[(index[2] * d[1] + index[1]) * d[0] + index[0]];
                for (let k = 0; k < 27; k++)
                    out[k] += p.coefficients[k] * weight;
            }
