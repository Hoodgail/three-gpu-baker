...
export function validateTexture(texture, name = 'texture') {
    if (!texture)
        return;
    ...
    if (!data.every(Number.isFinite))
        throw new TypeError(`${name}: nonfinite pixels.`);
...
function wrapIndex(i, size, mode = 'clamp') {
    if (mode === 'repeat')
...
export function sampleTexture(texture, uv) {
    if (!texture)
        return [1, 1, 1, 1];
    const m = texture.transform ?? [1, 0, 0, 0, 1, 0];
    let u = m[0] * uv[0] + m[1] * uv[1] + m[2], v = m[3] * uv[0] + m[4] * uv[1] + m[5];
    if (texture.flipY)
        v = 1 - v;
    const x = u * texture.width - 0.5, y = v * texture.height - 0.5, x0 = Math.floor(x), y0 = Math.floor(y), fx = x - x0, fy = y - y0;
    const out = [0, 0, 0, 0];
    for (let j = 0; j < 2; j++)
        for (let i = 0; i < 2; i++) {
            const index = (wrapIndex(y0 + j, texture.height, texture.wrapT) * texture.width +
                wrapIndex(x0 + i, texture.width, texture.wrapS)) *
                4;
            const w = (i ? fx : 1 - fx) * (j ? fy : 1 - fy);
            for (let k = 0; k < 4; k++)
                out[k] += texture.data[index + k] * w;
        }
    return out;
}
export function evaluateMaterial(material, uv) {
    const base = sampleTexture(material.map, uv), em = sampleTexture(material.emissiveMap, uv), metal = material.metalness ?? 0;
    return {
        albedo: (material.color ?? [1, 1, 1]).map((c, k) => clamp(c * base[k], 0, 1) * (1 - metal)),
        emission: (material.emissive ?? [0, 0, 0]).map((c, k) => Math.max(0, c * em[k] * (material.emissiveIntensity ?? 1))),
