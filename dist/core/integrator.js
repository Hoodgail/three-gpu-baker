import { add, sub, scale, mul, dot, cross, length, normalize, PI, clamp, cosineHemisphere, geometricNormal, maxComponent, powerHeuristic, mix3, } from './math.js';
...
function emissionLightPDF(scene, triangle, from, hitPoint, normal, doubleSided) {
    const delta = sub(hitPoint, from), d2 = dot(delta, delta), wi = normalize(delta);
 ...
 */
export function sampleDirect(scene, surface, rng, { spherical = false } = {}) {
    const count = scene.lights.length;
    if (!count)
        return { value: BLACK(), direction: [0, 1, 0] };
    const light = scene.lights[Math.min(count - 1, Math.floor(rng.next() * count))];
    const p = surface.p, n = spherical ? null : surface.n;
    let q = null, wi, dist = Infinity, radiance, area = 0, ln = null, emitter = false, twoSided = false;
    if (light.type === 'directional') {
        ...
        if (light.type === 'rect') {
            q = add(add(light.position, scale(light.u, 2 * rng.next() - 1)), scale(light.v, 2 * rng.next() - 1));
            ...
            const t = scene.triangles[light.triangle], r = Math.sqrt(rng.next()), v = rng.next(), w = [1 - r, r * (1 - v), r * v];
        ...
        const delta = sub(q, p);
        dist = length(delta);
        if (dist < scene.options.rayBias * 2)
            return { value: BLACK(), direction: [0, 1, 0] };
    ...
    if (area > 0) {
        ...
        if (twoSided)
            lightCos = Math.abs(lightCos);
        if (lightCos <= 1e-8)
            return { value: BLACK(), direction: wi };
        const pdf = (dist * dist) / (area * lightCos * count), bsdf = n ? cosine / PI : 1 / (4 * PI);
        ...
        if (light.type === 'spot') {
            const angle = light.angle ?? PI / 3, penumbra = light.penumbra ?? 0;
            attenuation *= smoothstep(Math.cos(angle), Math.cos(angle * (1 - penumbra)), dot(normalize(light.direction), scale(wi, -1)));
        }
        factor *= attenuation;
    }
    if (factor <= 0)
        return { value: BLACK(), direction: wi };
    const eps = scene.options.rayBias, origin = surface.g ? add(p, scale(surface.g, eps)) : p;
    const shadowDirection = q ? normalize(sub(q, origin)) : wi, shadowDistance = q ? length(sub(q, origin)) - eps : Infinity;
    if (trace(scene.bvh, origin, shadowDirection, shadowDistance, surface.triangle ?? -1, eps * 0.01, true))
        return { value: BLACK(), direction: wi };
...
export function traceIncoming(scene, surface, direction, rng, { initialPDF, bounces = scene.options.bounces } = {}) {
    let direct = BLACK(), indirect = BLACK(), throughput = [1, 1, 1], ao = 1;
    const eps = scene.options.rayBias;
    let origin = surface.g ? add(surface.p, scale(surface.g, eps)) : surface.p, previous = surface.p, skip = surface.triangle ?? -1, pdf = initialPDF ?? Math.max(0, dot(surface.n ?? [0, 0, 0], direction)) / PI;
    for (let depth = 0; depth <= bounces; depth++) {
        const hit = trace(scene.bvh, origin, direction, Infinity, skip, eps * 0.01);
        if (depth === 0)
            ao = !hit || hit.distance >= scene.options.aoDistance ? 1 : 0;
        if (!hit) {
            const contribution = mul(throughput, scene.environment);
            if (depth === 0)
                direct = add(direct, contribution);
            else
                indirect = add(indirect, contribution);
            break;
        }
        const s = surfaceAt(scene.bvh, hit, direction), m = evaluateMaterial(scene.materials[s.material], s.uv);
        if (s.backface && !m.doubleSided)
            break;
        if (maxComponent(m.emission) > 0) {
            const originalNormal = geometricNormal(scene.triangles[hit.index]);
            const lightPDF = emissionLightPDF(scene, hit.index, previous, s.p, originalNormal, m.doubleSided);
...
export function sampleIrradiance(scene, surface, rng) {
