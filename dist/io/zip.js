...
export function createZip(entries) {
    const parts = [], directory = [], seen = new Set();
    let offset = 0;
    for (const { name, data } of entries) {
        ...
        seen.add(name);
        const filename = utf8(name), bytes = typeof data === 'string' ? utf8(data) : bytesOf(data);
        if (filename.length > 65535 || bytes.length > 0xffffffff)
            throw new Error('ZIP32 limit exceeded.');
        const crc = crc32(bytes), local = new Uint8Array(30), l = new DataView(local.buffer);
    ...
    if (entries.length > 65535 || offset > 0xffffffff)
        throw new Error('ZIP32 limit exceeded.');
    const dir = concatBytes(directory), end = new Uint8Array(22), v = new DataView(end.buffer);
...
export function exportBakeBundle(result, { model, probes, includePFM = true, } = {}) {
    const entries = [
    ...
    if (model)
        entries.push({ name: 'model.glb', data: model });
    if (probes)
        entries.push({ name: 'probes.json', data: JSON.stringify(probes, null, 2) });
    entries.push({
        name: 'README.txt',
...
export function downloadBytes(bytes, name, type = 'application/octet-stream') {
    ...
    const url = URL.createObjectURL(new Blob([bytesOf(bytes).slice().buffer], { type })), a = document.createElement('a');
    a.href = url;
    a.download = name;
    a.click();
