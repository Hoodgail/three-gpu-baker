...
export async function prepareExportModel(source) {
    const root = source.clone(true), materials = [], textures = [], cache = new Map();
    const dispose = () => {
    ...
    async function portable(map) {
        ...
        if ('isDataTexture' in map || map.colorSpace === T.LinearSRGBColorSpace) {
            const d = (await extractTexture(map)), data = new Uint8Array(d.data.length);
            for (let i = 0; i < data.length; i++) {
                const v = d.data[i];
                if (!Number.isFinite(v) || v < 0 || v > 1)
                    throw new Error('Portable glTF material textures must be in [0,1]. Move HDR emission into emissiveIntensity before exporting.');
        ...
        if (!map.matrixAutoUpdate) {
            const e = map.matrix.elements, sx = Math.hypot(e[0], e[3]), det = e[0] * e[4] - e[1] * e[3];
            if (sx < 1e-12)
                throw new Error('Cannot export a singular texture transform.');
            out.center.set(0, 0);
