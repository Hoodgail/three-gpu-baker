...
export async function importModel(input, { format, baseURL = '', manager, configureLoader } = {}) {
    ...
    if (format === 'gltf' || format === 'glb') {
        ...
        const data = ArrayBuffer.isView(input)
            ...
            : input;
        return (await loader.parseAsync(data, baseURL)).scene;
    }
    if (format === 'obj') {
        ...
        if (typeof input !== 'string')
            throw new TypeError('OBJ input must be text or URL.');
...
export async function exportModel(root, { binary = true, ...options } = {}) {
    const { GLTFExporter } = await import('three/addons/exporters/GLTFExporter.js');
    let unsupported = false;
    root.traverse((o) => {
        const material = 'material' in o
            ? o.material
            : undefined;
        const list = Array.isArray(material) ? material : [material];
        if (list.some((m) => m && 'isNodeMaterial' in m))
            unsupported = true;
    });
    if (unsupported)
        throw new Error('Export the detached createModel() result BEFORE applyLightmaps(); node preview materials are not portable glTF materials.');
...
export async function importModelFiles(files, { entry, configureLoader } = {}) {
    const list = Array.from(files), byPath = new Map(), urls = [];
    const normalize = (path) => path.replaceAll('\\', '/').replace(/^\.\//, '');
    for (const file of list) {
        const name = normalize(file.webkitRelativePath || file.name);
        if (!name || byPath.has(name))
            throw new Error('Files need unique relative paths.');
    ...
    const name = entry ?? candidates[0], file = byPath.get(name);
    if (!file)
        throw new Error('Model entry is missing.');
    const { LoadingManager } = await import('three/webgpu');
    const manager = new LoadingManager();
    const base = 'https://lightmap-files.invalid/', baseURL = new URL(name.slice(0, name.lastIndexOf('/') + 1), base).href;
    manager.setURLModifier((url) => {
        if (url.startsWith('data:') || url.startsWith('blob:'))
            return url;
        const parsed = new URL(url, baseURL), path = normalize(decodeURIComponent(parsed.pathname.slice(1))), resource = byPath.get(path);
