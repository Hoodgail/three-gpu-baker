...
export class LightmapBaker {
    ...
    options;
    constructor(options = {}, compiler = new SceneCompiler()) {
    ...
    assertPrepared() {
        ...
        if (this.disposed)
            throw new Error('Baker is disposed.');
        if (!this.scene || !this.backend)
            throw new Error('Call and await prepare(scene) first.');
    }
    assertIdleOperation(name) {
        if (this.stepPromise ||
            this.runPromise ||
            this.readPromise ||
            this.resetPromise ||
            ...
            throw new Error(`${name} cannot run during another operation. Pause and await the current operation first.`);
        if (this.disposed)
            throw new Error('Baker is disposed.');
    }
    async prepare(input, { signal } = {}) {
        ...
        try {
            ...
            if ('isObject3D' in input)
                core = await (await import('./three/scene.js')).extractThreeScene(input, {
                    signal,
            ...
            const budget = this.options.maxMemoryMB ?? 512, estimate = this.options.width * this.options.height * 256;
            if (!Number.isFinite(budget) || budget <= 0 || estimate > budget * 1048576)
                throw new RangeError(`Estimated atlas working set ${(estimate / 1048576).toFixed(0)} MiB exceeds maxMemoryMB=${budget}. Lower resolutionScale or explicitly increase the budget.`);
            this.preparedScene = await this.compiler.compile(core, this.options);
            abortIfNeeded(signal);
            this.progressiveScheduler = new ProgressiveScheduler(this.options.width * this.options.height, this.options);
            let mode = this.options.backend;
            this.fallbackReason = null;
            if (mode === 'auto') {
                if (globalThis.navigator?.gpu || this.options.renderer)
                    mode = 'tsl';
                else {
                    mode = 'cpu';
                    this.fallbackReason =
            ...
            if (this.options.backendFactory)
                ...
                this.transportBackend = new CPUBackend();
            else if (mode === 'webgpu') {
    ...
    get progress() {
        return (this.scheduler?.progress(this.state) ?? {
            state: this.state,
            samples: 0,
    ...
    async step() {
        ...
        if (this.stepPromise)
            throw new Error('Concurrent step() calls are not allowed.');
    ...
    async bake({ signal, onProgress } = {}) {
        this.assertPrepared();
        if (this.runPromise)
            throw new Error('A bake() call is already running.');
        this.stop = false;
        const operation = (async () => {
            try {
                ...
                abortIfNeeded(signal);
                if (this.stop && !this.scheduler?.done)
    ...
    async getResult({ denoise = 'none', padding = false, signal } = {}) {
        this.assertPrepared();
        if (this.stepPromise)
            await this.stepPromise;
        abortIfNeeded(signal);
        if (!this.readPromise)
            this.readPromise = this.backend.read();
        let raw;
        try {
        ...
        const g = this.scene.gbuffer, n = g.width * g.height, direct = new Float32Array(n * 3), indirect = new Float32Array(n * 3), ao = new Float32Array(n), lightmap = new Float32Array(n * 3), sampleCounts = new Uint32Array(n);
        let minSamples = Infinity, maxSamples = 0;
        for (let i = 0; i < n; i++) {
            const a = i * 12;
        ...
        if (!direct.every(Number.isFinite) ||
            !indirect.every(Number.isFinite) ||
            !ao.every(Number.isFinite))
            throw new Error('Non-finite GPU output. Inspect the shader validation logs before exporting.');
        let result = {
            width: g.width,
            height: g.height,
        ...
        if (denoiseOptions && 'run' in denoiseOptions) {
            if (typeof denoiseOptions.run !== 'function')
                throw new Error('OptiX requires a configured offline runner. Use exportLightmap / three-gpu-baker-denoise or createOptixDenoiser().');
        ...
        if (padding)
            result = dilateChannels(result, this.options.padding);
        return result;
    }
    async generateProbes(options = {}) {
        this.assertPrepared();
    ...
    async dispose() {
        ...
        if (this.currentState === 'preparing')
            throw new Error('Await prepare() before disposing.');
        this.disposing = true;
        this.pause();
...
//# sourceMappingURL=LightmapBaker.js.map