...
export interface UVDiagnostic {
    code: 'missing' | 'nonfinite' | 'out-of-range' | 'degenerate' | 'overlap' | 'padding' | 'empty-chart';
...
export type BakeState = 'idle' | 'preparing' | 'ready' | 'baking' | 'paused' | 'complete' | 'resetting' | 'error' | 'disposed';
...
export interface ResolvedBakeOptions extends Required<Omit<BakeOptions, 'renderer' | 'device' | 'backendFactory' | 'texelDensity' | 'atlasProvider'>> {
    texelDensity?: number;
    atlasProvider?: AtlasProvider;
    renderer?: WebGPURenderer;
    device?: GPUDevice;
...
export interface ModelImportOptions {
    ...
    configureLoader?: (loader: import('three/addons/loaders/GLTFLoader.js').GLTFLoader | import('three/addons/loaders/OBJLoader.js').OBJLoader) => void;
}
