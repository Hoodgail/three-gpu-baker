...
const wrapCode = (w) => (w === 'repeat' ? 1 : w === 'mirror' ? 2 : 0);
/** All read-only scene data shares one vec4 storage binding (portable binding count). */
export function packScene(scene) {
    const l = SCENE_LAYOUT, triOffset = l.header, nodeOffset = triOffset + scene.triangles.length * l.triangle, materialOffset = nodeOffset + scene.bvh.nodes.length * l.node, lightOffset = materialOffset + scene.materials.length * l.material, textureOffset = lightOffset + scene.lights.length * l.light;
    ...
    if (total >= 16777216)
        throw new RangeError('Packed scene exceeds exact f32 integer indexing. Reduce scene/texture sizes.');
    const data = new Float32Array(Math.max(1, total) * 4);
    const put = (index, value) => data.set(value, index * 4);
    put(0, [
    ...
    scene.triangles.forEach((t, i) => {
        ...
        for (let k = 0; k < 3; k++)
            put(a + k, [...t.p[k], k === 0 ? (t.material ?? 0) : 0]);
        const normal = normalize(cross(t.p[1].map((v, k) => v - t.p[0][k]), t.p[2].map((v, k) => v - t.p[0][k])));
        for (let k = 0; k < 3; k++)
            put(a + 3 + k, [...(t.n?.[k] ?? normal), 0]);
    ...
    scene.materials.forEach((m, i) => {
        const a = materialOffset + i * l.material, mt = m.map?.transform ?? [1, 0, 0, 0, 1, 0], et = m.emissiveMap?.transform ?? [1, 0, 0, 0, 1, 0];
    ...
    scene.lights.forEach((light, i) => {
        const a = lightOffset + i * l.light, angle = ('angle' in light ? light.angle : undefined) ?? Math.PI / 3;
        put(a, [
            kinds[light.type],
        ...
        put(a + 1, [
            ...(('color' in light ? light.color : undefined) ?? [1, 1, 1]).map((v) => v * (('intensity' in light ? light.intensity : undefined) ?? 1)),
    ...
    for (const t of textures)
        data.set(t.data, textureMap.get(t) * 4);
    const g = scene.gbuffer, atlas = new Float32Array(g.width * g.height * l.atlas * 4);
...
export function checkStorageLimits(device, packed) {
    const limit = Math.min(device.limits.maxStorageBufferBindingSize, device.limits.maxBufferSize);
    for (const name of ['scene', 'atlas', 'result'])
        if (packed[name].byteLength > limit)
            throw new RangeError(`${name} buffer needs ${(packed[name].byteLength / 1048576).toFixed(1)} MiB; this device allows ${(limit / 1048576).toFixed(1)} MiB per storage binding. Lower resolutionScale or texture sizes.`);
}
