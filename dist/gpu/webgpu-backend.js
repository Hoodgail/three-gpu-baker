...
export class WebGPUBackend {
    ...
    async init(scene) {
        ...
        if (!this.device) {
            if (!globalThis.navigator?.gpu)
                throw new Error('WebGPU is unavailable. Use HTTPS/localhost and a WebGPU-capable browser, or select backend: "cpu".');
            const adapter = await navigator.gpu.requestAdapter({ powerPreference: 'high-performance' });
            if (!adapter)
                throw new Error('No WebGPU adapter.');
            this.device = await adapter.requestDevice();
        }
        const device = this.device, packed = packScene(scene);
        ...
        for (const name of ['scene', 'atlas', 'result']) {
            const data = packed[name], buffer = device.createBuffer({
                label: `lightmap/${name}`,
    ...
    async dispatch(start, count, sample) {
        ...
        device.queue.writeBuffer(this.params, 0, new Uint32Array([start, count, sample, this.scene.options.seed >>> 0]));
        const encoder = device.createCommandEncoder({ label: 'lightmap/tile' }), pass = encoder.beginComputePass();
    ...
    async read() {
        this.check();
        const device = this.device, size = this.packed.result.byteLength;
