...
export function chartCrops(result, { margin = 32 } = {}) {
    if (!Number.isInteger(margin) || margin < 0 || margin > 256)
        throw new RangeError('margin must be an integer from 0 to 256.');
    const { width, charts, coverage } = result, bounds = new Map();
    for (let i = 0; i < coverage.length; i++)
        if (coverage[i]) {
            const id = charts[i], x = i % width, y = Math.floor(i / width), b = bounds.get(id) ?? { id, minX: x, maxX: x, minY: y, maxY: y };
...
export function fillChart(result, crop) {
    const n = crop.width * crop.height, source = new Int32Array(n).fill(-1), queue = new Int32Array(n);
    let head = 0, tail = 0;
    ...
    while (head < tail) {
        const i = queue[head++], x = i % crop.width, y = Math.floor(i / crop.width);
        for (const j of [
            x > 0 ? i - 1 : -1,
    ...
    const indirect = new Float32Array(n * 3), ao = new Float32Array(n * 3), normal = new Float32Array(n * 3);
    for (let i = 0; i < n; i++) {
        const j = source[i];
        if (j < 0)
            throw new Error('Empty chart crop.');
        for (let c = 0; c < 3; c++) {
...
function execute(executable, args, { signal } = {}) {
    return new Promise((resolve, reject) => {
        ...
        child.once('close', (code) => code === 0 ? resolve() : reject(new Error(`OptiX worker exited ${code}: ${stderr}`)));
    });
}
export async function denoiseOffline(result, { executable = process.env.OPTIX_DENOISER_BIN, signal, onProgress, margin = 32, channels = ['indirect', 'ao'], } = {}) {
    ...
    if (!executable)
        throw new Error('Set OPTIX_DENOISER_BIN to the compiled native/optix/lightmap-optix executable.');
    executable = resolve(executable);
    await access(executable);
    const crops = chartCrops(result, { margin }), out = {
        ...result,
    ...
    try {
        for (let index = 0; index < crops.length; index++) {
            ...
            const crop = crops[index], images = fillChart(result, crop), paths = {};
            for (const key of ['indirect', 'ao', 'normal']) {
            ...
            for (const key of new Set(channels)) {
                ...
                for (let y = crop.minY; y <= crop.maxY; y++)
                    for (let x = crop.minX; x <= crop.maxX; x++) {
                        ...
                        if (!result.coverage[i] || result.charts[i] !== crop.id)
                            continue;
                        const j = (y - crop.minY + crop.margin) * crop.width + x - crop.minX + crop.margin;
                        if (key === 'ao')
                            out.ao[i] = Math.max(0, Math.min(1, (filtered.data[j * 3] + filtered.data[j * 3 + 1] + filtered.data[j * 3 + 2]) / 3));
                        else
                            for (let c = 0; c < 3; c++)
    ...
    for (let i = 0; i < out.lightmap.length; i++)
...
//# sourceMappingURL=OptixDenoiser.js.map