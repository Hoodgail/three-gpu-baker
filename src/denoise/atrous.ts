...
function validate(options: Types.SpatialDenoiseOptions) {
  const o: Required<Omit<Types.SpatialDenoiseOptions, 'signal'>> & { signal?: AbortSignal } = {
  ...
  for (const k of ['normalPower', 'positionSigma', 'colorSigma'] as const)
    if (!Number.isFinite(o[k]) || o[k] <= 0) throw new RangeError(`${k} must be positive.`);
...
export async function denoiseSpatial(
  result: Types.BakeResult,
  options: Types.SpatialDenoiseOptions = {},
) {
  ...
  if (o.type !== 'none') {
    ...
    for (let iteration = 0; iteration < (o.type === 'bilateral' ? 1 : o.iterations); iteration++) {
      ...
      const step: number = o.type === 'atrous' ? 2 ** iteration : 1,
        source = out.indirect,
        sourceAO = out.ao,
        target = source.slice(),
        targetAO = sourceAO.slice();
        ...
        for (let x = 0; x < result.width; x++) {
          ...
          for (let ky = -2; ky <= 2; ky++)
            for (let kx = -2; kx <= 2; kx++) {
              const xx: number = x + kx * step,
                yy: number = y + ky * step;
              if (xx < 0 || yy < 0 || xx >= result.width || yy >= result.height) continue;
              const j = yy * result.width + xx;
              if (!result.coverage[j] || result.charts[j] !== result.charts[i]) continue;
              let nd = 0,
                pd = 0,
                cd = 0;
              for (let c = 0; c < 3; c++) {
              ...
              aoWeightSum += aw;
              aoSum += aw * sourceAO[j];
              for (let c = 0; c < 3; c++) sum[c] += w * source[j * 3 + c];
            }
          if (o.channels.includes('indirect') && weightSum > 1e-20)
            for (let c = 0; c < 3; c++) target[i * 3 + c] = sum[c] / weightSum;
  ...
  for (let i = 0; i < n * 3; i++) out.lightmap[i] = out.direct[i] + out.indirect[i];
  out.metadata = {
    ...out.metadata,
    denoiser: {
      type: o.type,
