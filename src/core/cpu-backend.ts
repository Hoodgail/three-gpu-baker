...
export class CPUBackend {
  ...
  async dispatch(start: number, count: number, sample: number) {
    ...
    if (!scene || !output) throw new Error('Backend is not initialized.');
    const g = scene.gbuffer;
    for (let i = start; i < start + count; i++) {
      if (!g.coverage[i]) continue;
      const p = i * 3,
        surface = {
      ...
      const value = sampleIrradiance(
        scene,
        surface,
        new RNG(sampleSeed(i, sample, scene.options.seed)),
      );
      const offset = i * 12,
        n = output[offset + 3] + 1;
      for (let k = 0; k < 3; k++) {
