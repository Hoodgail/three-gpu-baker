...
export function validateOptions(options: Types.BakeOptions = {}) {
  ...
  for (const k of [
    ...
    if (!Number.isSafeInteger(out[k])) throw new TypeError(`${k} must be an integer.`);
  if (out.width < 8 || out.height < 8 || out.width > 8192 || out.height > 8192)
    throw new RangeError('Atlas dimensions must be 8..8192.');
  if (out.seed < 0 || out.seed > 4294967295) throw new RangeError('seed must be uint32.');
  if (out.samples < 1 || out.samples > 16777216)
    throw new RangeError('samples must be 1..16777216.');
  if (out.bounces < 0 || out.bounces > 8)
    throw new RangeError(
      'bounces must be 0..8; one segment is always traced for direct environment and emission.',
    );
  if (out.padding < 0 || out.padding > 32) throw new RangeError('padding must be 0..32.');
  ...
  for (const k of ['aoDistance', 'rayBias', 'targetDispatchMs'] as const)
...
const finiteVector = (v: unknown, n: number) =>
  Array.isArray(v) && v.length === n && v.every(Number.isFinite);
export function validateScene(scene: Types.CoreScene) {
  ...
  for (const [i, t] of scene.triangles.entries()) {
    ...
    if (triangleArea(t) < 1e-12) throw new Error(`Triangle ${i}: degenerate world-space triangle.`);
    if (
      t.n &&
      (!Array.isArray(t.n) ||
        t.n.length !== 3 ||
    ...
    for (const k of l.type === 'directional'
      ...
      : l.type === 'rect'
        ...
        : l.type === 'spot'
          ? ['position', 'direction']
          : ['position'])
      if (!finiteVector((l as unknown as Record<string, unknown>)[k], 3))
