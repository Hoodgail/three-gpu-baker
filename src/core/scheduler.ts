...
export class ProgressiveScheduler {
  ...
  commit(job: Types.Dispatch, elapsedMs: number) {
    if (
      job.start !== this.cursor ||
    ...
    const ratio = Math.max(
      0.5,
      Math.min(2, this.options.targetDispatchMs / Math.max(0.1, elapsedMs)),
    );
    this.tileSize = Math.max(
      this.options.minTileSize,
      Math.min(this.options.maxTileSize, Math.floor(this.tileSize * ratio)),
    );
  }
