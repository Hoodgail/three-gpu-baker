...
export async function extractTexture(map?: T.Texture | null, sourceUVChannel: Types.UVChannel = 0) {
  ...
  if (!image) throw new Error('Texture image is not loaded. Await the model/texture loader.');
  ...
  if (image.data) {
    ...
    if (![3, 4].includes(channels)) throw new Error('Textures must have three or four channels.');
  } else {
    const canvas =
    ...
    if (!canvas)
      throw new Error(
        'Image textures require browser Canvas; Node users can provide Float32 DataTextures.',
      );
    canvas.width = width;
  ...
  const out = new Float32Array(width * height * 4),
    byte = pixels instanceof Uint8Array || pixels instanceof Uint8ClampedArray,
    half = map.type === T.HalfFloatType;
  ...
  for (let i = 0; i < width * height; i++)
    for (let c = 0; c < 4; c++) {
      ...
      if (c < channels) {
        if (byte) value /= 255;
        else if (half) value = T.DataUtils.fromHalfFloat(value);
      }
      if (c < 3 && map.colorSpace === T.SRGBColorSpace) value = srgbToLinear(value);
      out[i * 4 + c] = value;
    }
...
export async function extractThreeScene(
  ...
  if (root instanceof T.Scene && root.environment)
    throw new Error(
      'HDR/cube environment importance sampling is not implemented. Use an explicit constant environment or emitting geometry.',
    );
  ...
  const triangles: Types.Triangle[] = [],
    ...
    sourceMaterials: (T.Material | null)[] = [],
    materialIds = new Map<T.Material | string, number>(),
    textureCache = new Map<T.Texture, ReturnType<typeof extractTexture>>(),
    lights: Types.BakeLight[] = [],
    environment = [0, 0, 0];
  const vector = (o: T.Object3D) => new T.Vector3().setFromMatrixPosition(o.matrixWorld).toArray();
  const materialID = async (source: T.Material) => {
    const m = source as SurfaceMaterial;
    ...
    if (
      ...
      throw new Error(
        `Material ${m.name || m.type}: alpha/transmission is not supported by this opaque diffuse baker.`,
      );
    if (m.vertexColors)
    ...
    if (m.isShaderMaterial || m.isNodeMaterial)
      throw new Error(
        `Material ${m.name || m.type}: custom shaders require an explicit core material descriptor.`,
      );
    ...
    if (m.side === T.BackSide)
      throw new Error(
        'BackSide materials must have their triangle winding reversed before baking.',
      );
    if (m.normalMap || m.bumpMap || m.displacementMap)
      throw new Error(
        'Bake normal/bump/displacement effects into geometry first; this integrator uses geometric/interpolated vertex normals.',
      );
  ...
  for (const object of objects) {
    ...
    if (object instanceof T.Light) {
      ...
      if (object instanceof T.HemisphereLight || object instanceof T.LightProbe)
        throw new Error(
          'HemisphereLight/LightProbe inputs need conversion to an environment; probe export is supported separately.',
        );
      if (object instanceof T.DirectionalLight || object instanceof T.SpotLight) {
        object.target.updateWorldMatrix(true, false);
        const p = vector(object),
          q = vector(object.target),
      ...
      } else throw new Error(`Unsupported light: ${object.type}`);
    }
    ...
    const list = Array.isArray(object.material) ? object.material : [object.material],
      ids = [];
    for (const m of list) ids.push(await materialID(m));
    ...
    const index = geometry.index,
      total = index ? index.count : position.count,
      start = geometry.drawRange.start ?? 0,
      end = Math.min(total, start + (geometry.drawRange.count ?? Infinity));
    if (start % 3 || end % 3) throw new Error('Triangle draw ranges must be multiples of three.');
    for (
      let instance = 0;
      instance < (object instanceof T.InstancedMesh ? object.count : 1);
      instance++
    ) {
      ...
      for (let offset = start; offset < end; offset += 3) {
        const group = geometry.groups.find(
          (g: { start: number; count: number; materialIndex?: number }) =>
            offset >= g.start && offset < g.start + g.count,
        );
        if (Array.isArray(object.material) && !group) continue;
        const mat = ids[Array.isArray(object.material) ? (group?.materialIndex ?? 0) : 0];
        if (mat === undefined) throw new Error('Geometry group references a missing material.');
        let vertices = [0, 1, 2].map((i) => (index ? index.getX(offset + i) : offset + i));
        if (flip) [vertices[1], vertices[2]] = [vertices[2], vertices[1]];
        const t: Types.Triangle = {
          p: vertices.map((i) =>
            new T.Vector3().fromBufferAttribute(position, i).applyMatrix4(matrix).toArray(),
          ),
          material: mat,
        ...
        if (uv) t.uv = vertices.map((i) => [uv.getX(i), uv.getY(i)]);
        if (uv1) t.lmUV = vertices.map((i) => [uv1.getX(i), uv1.getY(i)]);
...
function materialFromDescriptor(m: Types.DiffuseMaterial) {
  ...
  const make = (d?: Types.LinearTexture) => {
    if (!d) return null;
...
export function createThreeModel(prepared: Types.PreparedScene) {
  const group = new T.Group();
  group.name = 'Baked scene';
  const batches = new Map<string, Types.AtlasTriangle[]>();
...
export function createLightmapTextures(result: Types.BakeResult) {
  const make = (array: Float32Array, mono = false) => {
    const data = new Float32Array(result.width * result.height * 4);
    for (let i = 0; i < result.width * result.height; i++) {
      for (let c = 0; c < 3; c++) data[i * 4 + c] = array[i * (mono ? 1 : 3) + (mono ? 0 : c)];
      data[i * 4 + 3] = 1;
    }
...
export function applyLightmaps(
  ...
  { textures = createLightmapTextures(result) } = {},
) {
  root.traverse((o) => {
    ...
    const convert = (m: SurfaceMaterial) => {
      ...
      let albedo: import('three/src/nodes/core/Node.js').default<'vec3'> = vec3(
        ...(m.color?.toArray() ?? [1, 1, 1]),
      );
      ...
      if (m.emissiveMap) emission = emission.mul(texture(m.emissiveMap).rgb);
...
export function createThreeLightProbe(probe: Types.Probe) {
  if (probe.coefficients?.length !== 27) throw new Error('Expected 27 SH coefficients.');
