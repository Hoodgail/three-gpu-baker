...
export interface XAtlasUnwrapper {
  ...
  chartOptions: { useInputMeshUvs?: boolean };
  packAtlas(
...
export function createXAtlasProvider(unwrapper: XAtlasUnwrapper): AtlasProvider {
  let busy = false;
  return {
    async generate(triangles, options) {
      if (busy) throw new Error('This xatlas provider is already generating an atlas.');
      ...
      const oldPack = unwrapper.packOptions,
        oldChart = unwrapper.chartOptions;
      try {
        ...
        for (const indices of groups.values()) {
          ...
          g.setAttribute(
            ...
            new Float32BufferAttribute(
              indices.flatMap((i) => triangles[i].p.flat()),
              3,
            ),
          );
          g.setAttribute(
            ...
            new Float32BufferAttribute(
              indices.flatMap(
                (i) =>
                  (options.uvMode === 'repack' ? triangles[i].lmUV : triangles[i].uv)?.flat() ?? [
                    0, 0, 0, 0, 0, 0,
                  ],
              ),
              2,
          ...
          g.setAttribute(
            ...
            new Uint32BufferAttribute(
              indices.flatMap((i) => [i * 3, i * 3 + 1, i * 3 + 2]),
              1,
            ),
          );
        ...
        if (atlas.atlasCount !== 1 || atlas.width > options.width || atlas.height > options.height)
          throw new Error(
            'xatlas output does not fit one shared atlas. Increase resolution or lower density/padding.',
          );
        const output = triangles.map(() => new Array<number[]>(3));
        const seen = new Set<number>();
        for (const g of atlas.geometries) {
          const uv = g.getAttribute('uv2'),
            corners = g.getAttribute('bakerCorner');
          if (!uv || !corners || !g.index)
            throw new Error('xatlas omitted UVs or source mappings.');
          for (let i = 0; i < g.index.count; i++) {
            const vertex = g.index.getX(i),
              corner = corners.getX(vertex);
            if (
              !Number.isSafeInteger(corner) ||
              corner < 0 ||
        ...
        if (seen.size !== triangles.length * 3) throw new Error('xatlas omitted source triangles.');
        return output;
      } finally {
