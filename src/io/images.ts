...
export function encodePNG(rgba: Uint8Array, width: number, height: number) {
  ...
  for (let at = 0; at < scan.length; at += 65535) {
    const len = Math.min(65535, scan.length - at),
      h = new Uint8Array(5),
...
export function previewPixels(
  ...
  channel: Types.PreviewChannel = 'lightmap',
  { exposure = 1, toneMap = 'reinhard', transparent = true }: Types.PreviewOptions = {},
) {
  ...
  const out = new Uint8Array(result.width * result.height * 4),
    source = result[channel];
  for (let y = 0; y < result.height; y++)
    for (let x = 0; x < result.width; x++) {
      const i = y * result.width + x,
        target = ((result.height - 1 - y) * result.width + x) * 4;
      for (let c = 0; c < 3; c++) {
        let value = source[i * (channel === 'ao' ? 1 : 3) + (channel === 'ao' ? 0 : c)];
        if (channel === 'normals') value = value * 0.5 + 0.5;
        else if (channel !== 'ao') {
          if (channel !== 'albedo') {
            value *= exposure;
            if (toneMap === 'reinhard') value = value / (1 + Math.max(0, value));
          }
...
export function exportPNG(
  result: Types.BakeResult,
  channel: Types.PreviewChannel,
  options: Types.PreviewOptions = {},
) {
