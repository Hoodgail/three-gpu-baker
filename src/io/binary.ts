...
export const bytesOf = (input: Types.BinaryInput) =>
...
const channelEntries = Object.entries(channelSpec) as [
  keyof typeof channelSpec,
  readonly [keyof typeof constructors, number],
][];
...
export function exportLightmap(result: Types.BakeResult) {
  const n = dimensions(result.width, result.height),
  ...
  for (const [key, [type, size]] of channelEntries) {
    ...
    for (let i = 0; i < array.length; i++) {
      if (type === 'f32') view.setFloat32(i * 4, array[i], true);
      else if (type === 'i32') view.setInt32(i * 4, array[i], true);
      else if (type === 'u32') view.setUint32(i * 4, array[i], true);
  ...
  const start = 24 + description.length + ((4 - (description.length % 4)) % 4),
    total = start + data.length;
  if (total > 0xffffffff) throw new Error('TLMB exceeds 4 GiB.');
...
export function importLightmap(
  input: Types.BinaryInput,
  { maxBytes = 512 * 1024 * 1024 }: { maxBytes?: number } = {},
): Types.BakeResult {
  ...
  const d = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(json)),
    n = dimensions(d.width, d.height);
  ...
  for (const [key, [type, size]] of channelEntries) {
    ...
    const count = c.length * (type === 'u8' ? 1 : 4),
      end = c.offset + count;
    ...
    const array = new constructors[type](c.length),
      v = new DataView(data.buffer, data.byteOffset + c.offset, count);
    for (let i = 0; i < array.length; i++)
      array[i] =
        type === 'f32'
          ...
          : type === 'i32'
            ...
            : type === 'u32'
              ...
              : v.getUint8(i);
    if (type === 'f32' && !array.every(Number.isFinite)) throw new Error('Nonfinite TLMB channel.');
    (out as unknown as Record<string, ArrayBufferView>)[key] = array;
  }
  for (let i = 0; i < n; i++)
    if (
      out.coverage[i] > 1 ||
...
export function encodePFM(
  ...
  const header = utf8(`${channels === 3 ? 'PF' : 'Pf'}\n${width} ${height}\n-1.0\n`),
    out = new Uint8Array(header.length + data.length * 4),
    view = new DataView(out.buffer);
  out.set(header);
  for (let i = 0; i < data.length; i++) view.setFloat32(header.length + i * 4, data[i], true);
  return out;
}
export function decodePFM(input: Types.BinaryInput) {
  const bytes = bytesOf(input);
  let at = 0;
  const line = () => {
    if (at >= bytes.length) throw new Error('Truncated PFM header.');
    let end = at;
    while (end < bytes.length && bytes[end] !== 10) end++;
    if (end === bytes.length) throw new Error('Truncated PFM header.');
    const text = new TextDecoder().decode(bytes.subarray(at, end)).trim();
    at = end + 1;
    return text;
  };
  const magic = line();
  if (!['PF', 'Pf'].includes(magic)) throw new Error('Invalid PFM magic.');
  let dims = line();
  while (dims.startsWith('#')) dims = line();
  const parts = dims.split(/\s+/).map(Number);
  if (parts.length !== 2) throw new Error('Invalid PFM dimensions.');
  const [width, height] = parts,
    n = dimensions(width, height),
    channels = magic === 'PF' ? 3 : 1;
  let scaleText = line();
  while (scaleText.startsWith('#')) scaleText = line();
  const scale = Number(scaleText);
  if (!Number.isFinite(scale) || scale === 0) throw new Error('Invalid PFM scale.');
  if (bytes.length - at !== n * channels * 4) throw new Error('PFM payload length mismatch.');
  ...
  if (!data.every(Number.isFinite)) throw new Error('Nonfinite PFM data.');
