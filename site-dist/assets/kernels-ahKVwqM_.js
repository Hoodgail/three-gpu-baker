import{It as e,Nt as t}from"./index-Dtt7D38C.js";var n=Object.freeze({header:8,triangle:8,node:3,material:8,light:6,atlas:3,result:3}),r=e=>e===`repeat`?1:e===`mirror`?2:0;function i(i){let a=n,o=a.header,s=o+i.triangles.length*a.triangle,c=s+i.bvh.nodes.length*a.node,l=c+i.materials.length*a.material,u=l+i.lights.length*a.light,d=[],f=new Map,p=0;for(let e of i.materials)for(let t of[e.map,e.emissiveMap])t&&!f.has(t)&&(f.set(t,u+p),d.push(t),p+=t.width*t.height);let m=u+p;if(m>=16777216)throw RangeError(`Packed scene exceeds exact f32 integer indexing. Reduce scene/texture sizes.`);let h=new Float32Array(Math.max(1,m)*4),g=(e,t)=>h.set(t,e*4);g(0,[i.triangles.length,i.bvh.nodes.length,i.materials.length,i.lights.length]),g(1,[o,s,c,l]),g(2,[u,i.options.bounces,i.options.aoDistance,i.options.rayBias]),g(3,[...i.environment,i.options.maxRadiance]),i.triangles.forEach((n,r)=>{let i=o+r*a.triangle;for(let e=0;e<3;e++)g(i+e,[...n.p[e],e===0?n.material??0:0]);let s=e(t(n.p[1].map((e,t)=>e-n.p[0][t]),n.p[2].map((e,t)=>e-n.p[0][t])));for(let e=0;e<3;e++)g(i+3+e,[...n.n?.[e]??s,0]);g(i+6,[...n.uv?.[0]??[0,0],...n.uv?.[1]??[0,0]]),g(i+7,[...n.uv?.[2]??[0,0],0,0])}),i.bvh.nodes.forEach((e,t)=>{let n=s+t*a.node;g(n,[...e.min,e.escape]),g(n+1,[...e.max,e.count]),g(n+2,[e.first,0,0,0])});let _=e=>e?[f.get(e),e.width,e.height,r(e.wrapS)+4*r(e.wrapT)+(e.flipY?16:0)]:[0,0,0,0];i.materials.forEach((e,t)=>{let n=c+t*a.material,r=e.map?.transform??[1,0,0,0,1,0],i=e.emissiveMap?.transform??[1,0,0,0,1,0];g(n,[...e.color??[1,1,1],e.metalness??0]),g(n+1,[...(e.emissive??[0,0,0]).map(t=>t*(e.emissiveIntensity??1)),+!!e.doubleSided]),g(n+2,_(e.map)),g(n+3,_(e.emissiveMap)),g(n+4,[...r.slice(0,3),0]),g(n+5,[...r.slice(3,6),0]),g(n+6,[...i.slice(0,3),0]),g(n+7,[...i.slice(3,6),0])});let v={point:0,directional:1,spot:2,rect:3,emissive:4};i.lights.forEach((t,n)=>{let r=l+n*a.light,i=(`angle`in t?t.angle:void 0)??Math.PI/3;g(r,[v[t.type],(`triangle`in t?t.triangle:void 0)??-1,(`area`in t?t.area:void 0)??0,(`decay`in t?t.decay:void 0)??2]),g(r+1,[...((`color`in t?t.color:void 0)??[1,1,1]).map(e=>e*((`intensity`in t?t.intensity:void 0)??1)),(`distance`in t?t.distance:void 0)??0]),g(r+2,[...(`position`in t?t.position:void 0)??[0,0,0],Math.cos(i)]),g(r+3,[...e((`direction`in t?t.direction:void 0)??[0,-1,0]),Math.cos(i*(1-((`penumbra`in t?t.penumbra:void 0)??0)))]),g(r+4,[...(`u`in t?t.u:void 0)??[0,0,0],0]),g(r+5,[...(`v`in t?t.v:void 0)??[0,0,0],0])});for(let e of d)h.set(e.data,f.get(e)*4);let y=i.gbuffer,b=new Float32Array(y.width*y.height*a.atlas*4);for(let e=0;e<y.coverage.length;e++)b.set([...y.positions.subarray(e*3,e*3+3),y.coverage[e]],e*12),b.set([...y.normals.subarray(e*3,e*3+3),y.charts[e]],e*12+4),b.set([...y.geometricNormals.subarray(e*3,e*3+3),y.triangleIds[e]+1],e*12+8);return{scene:h,atlas:b,result:new Float32Array(y.width*y.height*12),offsets:{triOffset:o,nodeOffset:s,materialOffset:c,lightOffset:l,textureOffset:u}}}function a(e,t){let n=Math.min(e.limits.maxStorageBufferBindingSize,e.limits.maxBufferSize);for(let e of[`scene`,`atlas`,`result`])if(t[e].byteLength>n)throw RangeError(`${e} buffer needs ${(t[e].byteLength/1048576).toFixed(1)} MiB; this device allows ${(n/1048576).toFixed(1)} MiB per storage binding. Lower resolutionScale or texture sizes.`)}var o=`
...
`}export{i as a,a as i,o as n,c as r,s as t};